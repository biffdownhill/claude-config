...
The project's `Status` field MUST contain exactly these six options. If any are missing on first invocation, return `{"error": "backend_error", "message": "Status field options missing. Run /pm:init."}`.
