...
Cross-project lessons (a pattern/preference that applies everywhere) may go in `~/.claude/CLAUDE.md` regardless of manifest — but the bar is **deliberately high**: that file loads into *every* session, so each line is a permanent context cost. Add a lesson only if **all** of these hold — (a) a capable model gets it **wrong by default** (not derivable, not common knowledge); (b) it's **stable**, not platform/tool trivia that's better retrieved fresh from docs/README and would only go stale here; and (c) it isn't **already covered** by an existing lesson. **Default to not adding** — most findings don't clear the bar, and the right home for a tool fact is the relevant doc/README, not here. When a lesson genuinely clears the bar and you can edit the file, add it concisely in the right section. If it clears the bar but you **lack permission** to edit `~/.claude/CLAUDE.md`, do **not** silently drop it — state clearly at the **bottom of your response** that you wanted to record it (quote the lesson verbatim) but could not, so the user can add it themselves.
